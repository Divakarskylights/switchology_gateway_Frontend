...
const NodePalette = ({ meters = [] }) => {
  ...
  const meterNodesToDrag = meters.map(meter => ({
    id: String(meter.meterNo),
    text: meter.label || `Meter ${meter.meterNo}`,
    category: 'MeterNode',
  ...
  return (
    <div style={{ width: "100%", height: "100%", overflowY: "auto", backgroundColor: theme.palette.background.default }}>
      {/* Meters Section */}
      <Box sx={{ display: 'flex', flexDirection: 'column', alignContent: 'center' }}>
        <Box
          sx={paletteHeaderStyle}
          onClick={toggleMetersCollapse}
        >
          <Typography sx={{ fontSize: 16, fontWeight: "bold", color: theme.palette.text.primary }}>
            Meters
          </Typography>
          <IconButton onClick={(e) => { e.stopPropagation(); toggleMetersCollapse();}} size="small">
            {isMetersCollapsed ? <ExpandMoreIcon sx={{ color: 'success.main' }} /> : <ExpandLessIcon sx={{ color: 'error.main' }} />}
          </IconButton>
        </Box>
        <Collapse in={!isMetersCollapsed}>
          <Box sx={{ alignContent: 'center', p: 0.5 }}>
            {meterNodesToDrag.length > 0 ? meterNodesToDrag.map((nodeData) => (
              <DraggableNode key={nodeData.id} nodeGlobalData={nodeData} />
            )) : (
              <Typography variant="caption" sx={{ display: 'block', textAlign: 'center', p: 2, color: theme.palette.text.secondary }}>
      ...
      {/* Shapes Section */}
      <Box sx={{ display: 'flex', flexDirection: 'column', alignContent: 'center', mt: 1 /* Add some margin top */ }}>
        <Box
          sx={paletteHeaderStyle}
          onClick={toggleShapesCollapse}
        >
          <Typography sx={{ fontSize: 16, fontWeight: "bold", color: theme.palette.text.primary }}>
            Shapes
          </Typography>
          <IconButton onClick={(e) => { e.stopPropagation(); toggleShapesCollapse();}} size="small">
            {isShapesCollapsed ? <ExpandMoreIcon sx={{ color: 'success.main' }} /> : <ExpandLessIcon sx={{ color: 'error.main' }} />}
          </IconButton>
        </Box>
